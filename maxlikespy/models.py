import numpy as np
import maxlikespy.model as model
...
class PositionTime(model.Model):
...
class PositionGauss(model.Model):
...
class BoxCategories(model.Model):
    def __init__(self, data):
        super().__init__(data)
...
class TimeVariableLength(model.Model):
...
class AbsPosVariable(model.Model):
...
class RelPosVariable(model.Model):
...
class ConstVariable(model.Model):
